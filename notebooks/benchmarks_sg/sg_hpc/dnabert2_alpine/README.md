...
```text
dnabert2_alpine/src_patch/dnabert2_benchmark.py
```
...
If that zip is not available, put the three CSVs in `data/promoter_classification/` before uploading this folder to Alpine:

```text
dnabert2_alpine/data/promoter_classification/
```
...
Upload the whole `dnabert2_alpine/` folder to Alpine, for example:

```bash
/scratch/alpine/$USER/dnabert2_alpine
```
...
```bash
cd /scratch/alpine/$USER/dnabert2_alpine
```
